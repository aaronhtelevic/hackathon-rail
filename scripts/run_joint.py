...
from absolute import cold, harness, paths, solve, track  # noqa: E402
...
def main():
    ...
    ap.add_argument("--cold", action="store_true",
                    help="cold track: ignore meta.json's start station/time and infer them from "
                         "the first minute of cell towers (absolute/cold.py). Legs with no usable "
                         "towers are skipped — no fix beats a wild one.")
    a = ap.parse_args()
    legs = a.legs or paths.practice_legs()
    cfg = stream.StreamConfig(refit_every_s=a.refit_every, demo_speed=a.demo_speed)
    tr = "cold" if a.cold else "warm"
    g = track.load() if a.cold else None
    with RunWriter("joint", lane="joint", track=tr, run_id=a.run_id, notes=a.notes) as run:
        for leg in legs:
            lw = run.leg(leg)
            ws = None
            if a.cold:
                ws, ranked = cold.cold_start(leg, g)
                if ws is None:
                    lw.event("C1", "no cold fix (no usable cell towers) — leg skipped",
                             level="warn", pct=1.0)
                    lw.done("skipped: no cold fix")
                    print(f"{leg[:44]:44s} cold: no fix")
                    continue
                lw.event("C1", f"cold start {ws.station_name} @ {ws.t0_ms} "
                               f"({len(ranked)} station hypotheses)", pct=0.05)
            if a.stream:
                info = stream.solve_leg(leg, a.team, ws=ws, gui=lw, cfg=cfg, sub_track=tr)
            else:
