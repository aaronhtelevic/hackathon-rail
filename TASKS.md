...
| station call detected / median \|timing\| | 0/1 | 40/50, 17 s | **43/50**, 19 s — of the 7 misses, 5 are **unscoreable for anyone** (ground truth stops recording before ever reaching 500 m from the end — not a mid-leg gap, a trailing GPS loss near the destination), 1 is the same-minute wrong-route pick (`ic2809_04`), 1 is a real timing miss on a *bad*-GT leg (`ic536_01`, 124 s) |
...
| T5 | Validate against the scorer across practice legs | M | DONE | After the N5 schedule-check fix: **43/50 detected.** Of the 7 misses: **5 have `referenceTimeMs = None`** because ground truth stops recording before ever reaching 500 m from the destination — the last recorded fix is 20–75 % of the way along the leg, 0–225 s before the sensors themselves stop (`ic2809_04's` other issue, `ic2933_03/04`, `ic3013_01`, `ic830_05`, `s51_761_04`). This is a trailing GPS-loss-near-arrival pattern (all cluster around large/urban stations — Antwerpen-Berchem, Brussel-Zuid throat, Linkeroever, Brussels-Airport), not a mid-leg tunnel gap, and it is **unscoreable for anyone** — no submission can earn this point, by construction of `metrics.find_time_at_distance`. 1 is the same-minute wrong-route pick (`ic2809_04`, name mismatch). 1 is a real, scoreable timing miss on a *bad*-GT leg (`ic536_01`, 124 s vs ±90 s) — see WORKLOG.md → "Station-call misses" for the diagnosis. Nothing left to win on the 5 unscoreable legs; `ic536_01` is the only remaining lever, and it is small (1 leg). |
