...
{:else}
  <div class="grid">
    ...
    <div class="cell">
      <span class="k dim">station call</span>
      <span class="v" class:ok={station?.detected} class:bad={station?.detected === false}>
        {station?.detected == null ? '—' : station.detected ? 'hit' : 'miss'}
      </span>
      <span class="sub dim">Δt {num(station?.timingErrorS, 0)}s (±90 s)</span>
    </div>
    {#if firstFix}
      <div class="cell">
        <span class="k dim">first fix</span>
        <span class="v">{num(firstFix.timeToFirstFixS, 0)} s</span>
        <span class="sub dim">err {num(firstFix.errorM)} m (&lt;1000 m)</span>
      </div>
    {/if}
  </div>
{/if}
